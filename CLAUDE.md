...
Downloaded/cached data lands under a gitignored top-level `data/` directory (curated sample: up to ~100 SpaceNet tiles across Vegas, Khartoum, Paris, and Shanghai — `spacenet_data_prep.ipynb` targets 25 tiles/city but the actual count depends on how many road-tile candidates pass the building-overlap filter per city — plus 15 Potsdam crops). No cloud compute needed — everything here is a local, curated sample fetched via plain HTTPS. Trained model checkpoints (see `train_unet.ipynb` below) land under a separate gitignored top-level `models/` directory, kept out of git for the same reason (`data/` and `models/` are both binary artifacts, not source).
